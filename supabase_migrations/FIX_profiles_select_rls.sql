-- ⚠️ DEPRECATED: This file has been superseded by FIX_profiles_rls_no_recursion.sql
-- ⚠️ DO NOT USE THIS FILE - It contains a query that can cause infinite recursion
--
-- The correct fix is in FIX_profiles_rls_no_recursion.sql which uses:
-- - SECURITY DEFINER functions for SELECT policies
-- - Direct companies table queries for INSERT/UPDATE policies
--
-- This file is kept for reference only. The problematic query is on line 24-25:
--   SELECT company_id FROM profiles WHERE id = auth.uid()  -- ❌ Causes recursion!
--
-- Original comment:
