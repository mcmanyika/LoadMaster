        ...
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col">
          ...
          <div className="p-8">
            ...
            {!isSupabaseConfigured && (
               <div className="mb-6 p-3 bg-amber-50 border border-amber-200 rounded-lg flex items-start gap-2 text-amber-800 text-xs">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                {isLogin && !isSupabaseConfigured && (
                   <p className="text-xs text-slate-400 mt-1">Demo password: <span className="font-mono bg-slate-100 px-1 rounded text-slate-600">password</span></p>
                )}
              </div>
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-slate-900 hover:bg-slate-800 text-white font-medium py-2.5 rounded-lg transition-colors mt-6 disabled:opacity-50 disabled:cursor-not-allowed flex justify-center items-center"
              >
                {loading ? (
            ...
            {!isSupabaseConfigured && (
              <div className="mt-6 pt-6 border-t border-slate-100 text-center">
                ...
                <div className="flex gap-2 justify-center">
                   <button onClick={() => { setEmail('owner@demo.com'); setPassword('password'); }} className="text-xs bg-slate-100 hover:bg-slate-200 px-2 py-1 rounded text-slate-600">Owner</button>
                   <button onClick={() => { setEmail('driver@demo.com'); setPassword('password'); }} className="text-xs bg-slate-100 hover:bg-slate-200 px-2 py-1 rounded text-slate-600">Driver</button>
                </div>
              </div>
            )}
